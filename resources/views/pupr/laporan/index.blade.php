...
@section('title', 'Daftar Laporan Tervalidasi - SIGAP')

@section('content')
<div class="card card-sigap p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="fw-bold text-primary mb-0">Daftar Laporan Tervalidasi (Kecamatan)</h3>
    </div>
        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span class="badge bg-{{ $item->tingkat_bahaya == 'Bahaya Tinggi' ? 'danger' : ($item->tingkat_bahaya == 'Sedang' ? 'warning' : 'info') }}">
                            {{ $item->tingkat_bahaya }}
                        </span>
                    </td>
                    <td>
                        ...
                        @endphp
                        <span class="status-badge {{ $statusClass }}">{{ $item->status->nama_status ?? 'Valid (Kecamatan)' }}</span>
                    </td>
                    <td>
                        <a href="{{ route('pupr.laporan.show', $item->id_laporan) ?? '#' }}" class="btn btn-sm btn-outline-primary px-3">Tindak Lanjut</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="text-center py-5">
                        <p class="text-muted mb-0">Belum ada laporan tervalidasi yang perlu ditindaklanjuti.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
